...
def get_product(product_id: int, db: Session = Depends(get_session)):
    ...
    stock_total = sum(db.exec(
        select(func.sum(StockMovement.quantity))
        .where(StockMovement.sku_id.in_([s.id for s in product.skus]))
    ).all()) or 0 # Wait, sum([None]) is bad, SQL sum is better

    # Correct way to get total stock for all SKUs
