...
@router.get("/", response_model=List[HomepageSection])
...
@router.get("/admin", response_model=List[HomepageSection])
def get_cms_admin(page: str = "homepage", db: Session = Depends(get_session)):
...
async def create_section(section: HomepageSectionCreate, db: Session = Depends(get_session)):
...
async def update_section(section_id: int, section: HomepageSectionUpdate, db: Session = Depends(get_session)):
...
async def delete_section(section_id: int, db: Session = Depends(get_session)):
    """Elimina una sección."""
    db_section = db.get(HomepageSection, section_id)
    if not db_section:
        raise HTTPException(status_code=404, detail="Sección no encontrada")
...
async def reorder_sections(request: HomepageReorderRequest, db: Session = Depends(get_session)):
...
def get_help_sections_admin(db: Session = Depends(get_session)):
...
async def create_help_section(section: HelpSectionCreate, db: Session = Depends(get_session)):
...
async def update_help_section(section_id: int, section: HelpSectionUpdate, db: Session = Depends(get_session)):
...
async def delete_help_section(section_id: int, db: Session = Depends(get_session)):
    db_section = db.get(HelpSection, section_id)
    if not db_section: raise HTTPException(status_code=404)
    db.delete(db_section)
    db.commit()
...
async def reorder_help_sections(request: HelpReorderRequest, db: Session = Depends(get_session)):
