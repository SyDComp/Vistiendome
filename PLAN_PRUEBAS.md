...
| H12 | Portada › modal de bienvenida | La imagen del modal **está rota** (`naturalWidth = 0`). Apunta a `lh3.googleusercontent.com/gps-cs-s/...`, una URL temporal del perfil de Google Business que ya expiró. Es lo primero que ve una visitante nueva. | ✅ Corregido — el modal ya no muestra imagen rota nunca. **Y Allan cargó una foto de verdad el 2026-08-31**: verificado en el navegador, el modal muestra una prenda |
| H18 | **Producción › Mesa de Trabajo** | **Editar precios en masa deshacía ventas, en silencio.** La pantalla carga las variantes (que traen su `stock`) y al guardar devuelve el producto entero. El servidor comparaba ese stock contra el real y, si diferían, creaba un ajuste para "corregirlo" — con el valor que la pantalla había cargado al abrirse. Reproducido: stock 10 → se vende 1 → 9 → Paola guarda **sólo un precio** → vuelve a **10**, con un ajuste `+1` y la nota "Ajuste manual desde edición masiva de producto". | ✅ Corregido en los dos lados: el campo `stock` pasa a ser **opcional** en el servidor (ausente = "no lo toques") y la Mesa de Trabajo dejó de mandarlo, porque no lo gestiona. Verificado con las dos contrapruebas: sin stock el saldo queda en 9; declarándolo, el ajuste sigue funcionando (10 → 25) |
| H19 | Producción › Mesa de Trabajo | Si la carga fallaba, el error sólo iba a la consola y la pantalla decía *"No se encontró el lienzo solicitado. Verifica el nombre"* — le echaba la culpa a Paola de una falla del servidor. Y el mismo mensaje aparecía con el catálogo vacío y con una búsqueda sin resultados, que no son lo mismo. | ✅ Corregido — estado de error propio con el motivo real, y el vacío distingue "no hay productos" de "tu búsqueda no coincide" |
| H20 | Servidor › listado de variantes | Quedó un `print` de depuración (`--- DEBUG WORKSPACE ---`) en el endpoint de SKUs: escribía en el registro del servidor en cada carga de la Mesa de Trabajo. | ✅ Quitado |
| H16 | Tienda › checkout con retiro | **Ningún pedido de retiro hecho desde la web entró nunca al sistema.** Al no elegir comuna, el formulario mandaba `comuna_id: ""`, que no parsea como entero: `422`. Y no se veía, porque el registro en CRM se dispara sin esperar respuesta — la clienta mandaba su pedido por WhatsApp y el pedido no existía. Verificado: el único pedido de retiro en la base tiene `origen = MANUAL`. | ✅ Corregido en los dos lados: el cliente manda `null`, y el servidor acepta `""` como nulo venga de donde venga |
| H17 | Tienda › checkout | Un RUT más largo de 12 caracteres devuelve **500** y el pedido se pierde, también en silencio. Además el carrito se vacía igual, aunque el registro haya fallado. Salió al equivocarme escribiendo en una prueba. | ✅ Corregido en tres frentes: **dígito verificador** del RUT (módulo 11) en el formulario, largos de columna validados en el cliente **y** en el servidor (500 → 422 con motivo), y **el carrito ya no se vacía si el registro falla** — se avisa y los productos quedan para reintentar |
